...
SaveGameData CaptureSnapshot(const gm::Scene& scene,
    ...
    data.cameraFov = gameplay.GetFovDegrees();
    data.worldTime = gameplay.GetWorldTimeSeconds();
    return data;
}
...
void ApplySnapshot(const SaveGameData& data,
    ...
    // Scene loading deferred; this scaffold only logs the intention.
    gm::core::Logger::Info("[SaveManager] ApplySnapshot scene='%s' (future implementation)",
                           data.sceneName.c_str());
    (void)scene;
    (void)camera;
    (void)gameplay;
}
